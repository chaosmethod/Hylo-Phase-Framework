...
$$
   ...
   This lemma is **proved internally within HPF canon**. The geometric input $f_{coh}$ is substrate-native; the one-square-root transfer is the active HPF governor map. This does not claim assumption-free uniqueness outside HPF.
...
|$\alpha_{vac}=\sqrt{f_{coh}}$|Lemma|Section 6|Proved internally within HPF canon under the active governor law|
...
|$\alpha_{vac}=\sqrt{f_{coh}}=0.737125$|Post-governor vacuum availability|One-step governor transfer from coherent support to local availability|Proved internally within HPF canon|Section 6|Sets usable vacuum fraction and DM complement|
...
**Status:** $b$ and $f_{coh}$ derived / substrate-native; $\alpha_{vac}=\sqrt{f_{coh}}$ proved internally within HPF canon; $\Omega_{\rm dm}$ downstream output.
...
\left[S+\frac{1}{k}\ln!\bigl(1+e^{-k(S-1.05)}\bigr)\right]*{S*{\rm lo}}^{S_{\rm hi}}.
$$
Hence
$$
I
