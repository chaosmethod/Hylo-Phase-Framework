...
- **HPF (Hylo Phase Framework)** — legality, regime detection, and execution constraints
...
| $n$ | Radial shell index | $R_H/L_{vac} = \varphi^n$ | Active shell count is $n=220$ in current active canon |
...
The active Lambda and dark matter derivation package is: `Volume_IV_Lambda_and_Dark_Matter.md`
...
## 12.x Candidate Phenomenology Symbols — Stochastic Jitter Note

These symbols belong to the separate candidate note
`HPF_Stochastic_Jitter_Falsifier_Candidate_Note.md`.
They are not part of the active \(\Lambda\)/dark-matter derivation chain and should not be used there without an explicit bridge derivation.

| Symbol | Meaning | Status | Notes |
|---|---|---|---|
| \(t_{sched}\) | Scheduler tick | Active identity already live | \(t_{sched}=L_{vac}/c\) |
| \(\sigma_t\) | Propagation-induced arrival-time broadening | Candidate phenomenology symbol | Observable targeted by jitter falsifier |
| \(N_{\rm eff}\) | Effective number of sampled transport domains | Candidate placeholder | Not yet derived from canon-safe transport law |
| \(L_{\rm corr}\) | Effective propagation correlation length | Candidate placeholder | Used in scaling ansatz only |
| \(\Gamma(S_f;b,\eta)\) | Regime gate for jitter activation | Candidate phenomenology object | Controlled by the common spine \(\{S_{\rm blur},\eta,b\}\) |
| \(\Xi(b,\eta)\) | Dimensionless substrate prefactor | Candidate phenomenology object | Open obligation |

**Status note.** These symbols are introduced for a separate external-falsifier lane. They do not modify the active Volume IV derivation package.
...
3. whether it is derived, anchored, candidate-locked, or proved internally within HPF canon,
