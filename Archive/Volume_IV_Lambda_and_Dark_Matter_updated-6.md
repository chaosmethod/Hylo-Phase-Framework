...
S\_{\\rm cap}=5.7889.
   ...
   **Four-term derivation (candidate, 2026-03-31, updated v3):**
...
= 1.05 + 4.712389 + 0.020833 + 0.005750 = 5.788972
   ...
   Note: (b^3/(2\phi-1)^2 = b^3/5) numerically. The form (2\phi-1)^2) is preferred because (2\phi-1 = P'(\phi) = \sqrt{5}) is the derivative of the Fibonacci characteristic polynomial (P(x)=x^2-x-1) at its root. The denominator is the squared fixed-point sensitivity of the Fibonacci recursion — the Binet normalization factor. This is not an arbitrary integer.

   **Fork B eliminated (2026-03-31):** \zeta(S_{\rm budget}) = 0. No collapse-lag mechanism possible. \delta_{\rm collapse} is geometric (Fork A confirmed).

   **Status:** Denominator candidate-explained (strong). Full collapse term: leading candidate, not yet proved. Cubic order: candidate mechanism established via 3D structural manifold argument (from robustness paper), near-theorem structural derivation — three obligations remaining: constitutive separability, k∝1/b bridge, lower-order exclusion theorem. See Reference/Delta_Collapse_Theorem_Note_v5.md.

   **Guardrail:** Any accepted derivation must be independent of 5.7889 and must preserve consistency elsewhere in canon.
