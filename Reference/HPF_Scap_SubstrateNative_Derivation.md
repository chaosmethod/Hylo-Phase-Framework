...
**Status:** Derived / substrate-native (under candidate cell-counting premise)  
...
$$
S_{\rm cap} \quad \text{is derived / substrate-native, under a candidate cell-counting premise.}
$$

The probe discovered the value first. The later closure showed the substrate was always going to demand it. The standalone first-principles closure of the cell-counting premise (the BCC Fibonacci spiral arc-coverage argument) remains the named frontier obligation; see §4.4 and §8.
...
**Status of the underlying cell-counting premise.** The static identity rests on a cell-counting picture: the BCC Fibonacci spiral covers \(N_s^2\) lattice cells per full causal arc, divided by the per-shell golden-ratio gain \(\varphi^2\). The cell-counting premise is currently stated in the QPRCA v0.3.0 module docstring rather than a standalone derivation note. The static identity yields \(n=220\) with the rounding margin above, but the cell-counting premise (\(N_s^2\) cells per causal arc) remains an open sub-derivation. Accordingly, \(n=220\) is treated here as **derived / substrate-native under a candidate cell-counting premise**, pending standalone closure of the arc-coverage argument. This is the framework's named frontier obligation on the \(S_{\rm cap}\) chain.
...
## New live wording (2026-05-09)
- \(S_{\rm cap}\) discovered historically by probe,
- now **derived / substrate-native** in the live package, under a candidate cell-counting premise,
...
- Standalone first-principles closure of the cell-counting premise is named frontier obligation.
...
- promote \(n=220\) from "derived under candidate cell-counting premise" to "fully derived without premise" — that promotion requires standalone closure of the BCC Fibonacci spiral arc-coverage argument, currently the named frontier obligation,
- promote \(n=220\) from geometry-native to theorem-level uniqueness of the rounding.

It closes one status question only: the live status of \(S_{\rm cap}\), under explicit identification of the remaining sub-derivation that would close the chain fully.
...
> $$
> 
> The static shell-count identity is the primary load-bearing closure with rounding margin ≈ 0.5 in \(n_{\rm raw}\) units. The dynamic integral form \(n_{\rm sel}=\mathrm{round}[(24/\ln\varphi)\int(1-\zeta)dS]\) is a consistency check that yields the same \(n=220\) at \(n_{\rm raw}=219.742\), with rounding band 5.7841 ≤ \(S_{\rm cap}\) ≤ 5.8041 of width \(\ln\varphi/N_s\approx 0.020\). The step-limit formula gives 5.7917; the probe gave 5.7889. Both sit inside the integral form's rounding band. The 0.003 difference is sub-band by construction and does not need to be resolved by the framework. The prior shell-width-as-resolution explanation (which incorrectly placed a band centered on 5.7917 with lower edge 5.7816) is retired as of 2026-05-09. No value of \(k\) in the logistic gate explains the gap — the integral has a floor at 5.7917 and lower \(k\) raises \(S_{\rm cap}\) further from the probe, not toward it. The correct live truth-status is: \(S_{\rm cap}\) is derived / substrate-native under a candidate cell-counting premise, with no free parameters, and the probe is preserved as the original discovery path. Standalone first-principles closure of the cell-counting premise (\(N_s^2\) cells per causal arc) is the named frontier obligation; closing it would remove the "under candidate premise" qualifier.
