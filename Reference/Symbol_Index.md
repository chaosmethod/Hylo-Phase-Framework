...
## Canonical Reference — Milestone Lock State (2026-04-10; L_vac/L_Λ disambiguation patch 2026-05-08)
...
| Symbol | Name | Definition | Notes |
|---|---|---|---|
| \(L_{\rm vac}\) | Substrate resolution scale | Lattice site spacing; \(L_{\rm vac}=c\,t_{\rm sched}\) | Active radial law: \(L_{\rm vac}=R_H/\varphi^n\); no `/2` in the radial identity. Substrate-level scale \(\approx 1.45\times 10^{-20}\,\mathrm{m}\). **Distinct from \(L_{\Lambda}\)** (macroscopic Λ-length scale, see entry below); in HPF Λ-derivations the proportionality \(\Lambda \propto 1/L^2\) carries the substrate-level multiplicity correction sourced at \(L_{\rm vac}^{2}\) (HPF_Lvac_Squared_Bipartite_Derivation §§ 5–7), while the dimensional content of \(\Lambda\) is carried at \(L_{\Lambda}^{2}\) (Volume IV § 12) |
| \(L_{\Lambda}\) | Cosmological-constant length scale | Macroscopic length carrying \(\Lambda\)'s dimensional content; defined through \(\Lambda \sim 1/L_{\Lambda}^{2}\) (Volume IV § 12) | Active branch value \(L_{\Lambda}\approx 9.6\times 10^{25}\,\mathrm{m}\); equivalently \(L_{\Lambda}=R_H/\sqrt{3\,\Omega_{\Lambda}}\approx R_H/1.44\) under the Friedmann form \(\Lambda\,R_H^{2}=3\,\Omega_{\Lambda}\). Distinct from substrate \(L_{\rm vac}\) by ~46 orders of magnitude. The (1−4b/72) substrate-level multiplicity correction (HPF_Lvac_Squared_Bipartite_Derivation §§ 5–7) is sourced at \(L_{\rm vac}^{2}\) and propagates through the inverse-square relationship to give \(\Lambda_{\mathrm{corrected}}=\Lambda_{\mathrm{raw}}/(1-4b/72)\) at the macroscopic level (§ 7.1). External cross-cites that write "\(L_{\rm vac}=l_H\)" (e.g. Pascher Doc 206 § 8.2) use "\(L_{\rm vac}\)" as a stand-in for \(L_{\Lambda}\); they recover the Friedmann form but do not identify the substrate scale |
