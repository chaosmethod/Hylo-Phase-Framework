...
3. deriving the optimal CHSH measurement angle $\pi/8$ from substrate geometry,
...
The candidate-locked 4-bit grammar $(n_L, n_R, b_{\rm bit}, q)$ contains a directional qubit in the $(n_L, n_R)$ sector. The two resolved address states are:

$$|L\rangle \equiv (n_L=1,\ n_R=0), \qquad |R\rangle \equiv (n_L=0,\ n_R=1).$$

The bifurcated address state $(n_L=1, n_R=1)$ is the superposition that lives in the active entropy band $S_{\rm ent} < S_f < S_{\rm cap}$. The vacuum state $(0,0)$ is null. The entanglement-active subspace is $\{|L\rangle, |R\rangle\}$ — a qubit embedded in the 4-state directional space.
...
$M^2 = -I$ is confirmed. This is the spin-$\frac{1}{2}$ time-reversal structure — the algebraic engine behind Kramers degeneracy and the non-factorizability of the Bell singlet. The $M^2 = -I$ property forces the entangled pair to be non-factorizable: the bifurcated state cannot be smoothly deformed into a product state without breaking the antisymmetry.
...
$$\Delta\theta_{\rm sector} = \frac{2\pi}{24} = \frac{\pi}{12}.$$
...
3. bipartite symmetry ($|A| = |B|$) → $B$-origin fraction is exactly $1/2$,
...
## 4.1 The optimal CHSH angle is $\pi/8$

The peak correlation separation on the 24-sector ring is $3/2$ sectors. In radians:

$$\theta_{\rm peak} = \frac{3}{2} \times \frac{\pi}{12} = \frac{\pi}{8}.$$
...
$$a = 0, \quad a' = \frac{\pi}{4}, \quad b = \frac{\pi}{8}, \quad b' = \frac{3\pi}{8}.$$

The angular separations between measurement bases are all $\pi/8$ — the same angle determined by the substrate geometry.

## 4.3 The Tsirelson bound

With these settings:

$$S_{\rm max} = 4 \times \cos\!\left(\frac{\pi}{8}\right) \times \sin\!\left(\frac{\pi}{8}\right) \times 2 = 2\sqrt{2}.$$

More directly: each of the four $E$ terms contributes $\cos(\pi/8)$ or $\cos(3\pi/8) = \sin(\pi/8)$, and the standard CHSH optimization gives:

$$S_{\rm max} = 2\sqrt{2} \approx 2.828.$$
...
$$M = JK \xrightarrow{M^2=-I} \text{Bell singlet structure} \xrightarrow{\text{bipartite reach } 3/2} \theta_{\rm peak} = \frac{3}{2} \times \frac{\pi}{12} = \frac{\pi}{8} \xrightarrow{\text{CHSH at } \pi/8} S_{\rm max} = 2\sqrt{2}$$
...
$$L_{\rm eff}^{\rm corrected} = \frac{3}{2} + \frac{b}{72}.$$

## 5.2 Corrected peak angle

$$\theta_{\rm peak}^{\rm corrected} = \left(\frac{3}{2} + \frac{b}{72}\right) \times \frac{\pi}{12} = \frac{\pi}{8} + \frac{b\pi}{864}.$$

## 5.3 Corrected Tsirelson bound

The CHSH value at the corrected angle shifts from $2\sqrt{2}$ by a first-order correction. Expanding to leading order in $\delta\theta = b\pi/864$:

$$\frac{dS_{\rm max}}{d\theta}\bigg|_{\theta=\pi/8}$$

must be evaluated. Because $\pi/8$ is the optimal angle, $S_{\rm max}$ is at a maximum there and the first derivative vanishes. The leading correction is second order:

$$\Delta S \sim -\frac{1}{2}\left|S''\right| \left(\frac{b\pi}{864}\right)^2.$$

The second derivative of the CHSH correlator at the optimal point is $|S''| = 2\sqrt{2}$, giving:

$$\Delta S = -\sqrt{2}\left(\frac{b\pi}{864}\right)^2.$$

Numerically:

$$\frac{b\pi}{864} = \frac{0.3063 \times 3.1416}{864} = 0.001114,$$

$$\Delta S = -\sqrt{2} \times (0.001114)^2 = -1.756 \times 10^{-6}.$$

## 5.4 Predicted corrected bound

$$S_{\rm max}^{\rm corrected} = 2\sqrt{2} - 1.76 \times 10^{-6} \approx 2.8284253.$$

vs the uncorrected:

$$2\sqrt{2} \approx 2.8284271.$$

The predicted deviation is $\sim 1.8 \times 10^{-6}$ — approximately $0.6$ parts per million below the standard Tsirelson bound.
...
The predicted deviation of $\sim 1.8 \times 10^{-6}$ from $2\sqrt{2}$ is well below the precision of any current Bell test experiment. State-of-the-art loophole-free Bell tests achieve CHSH values with uncertainties of order $10^{-2}$ to $10^{-3}$, roughly three orders of magnitude above the predicted correction.
...
|---|---|---|---|---|
| Dark matter fraction $\Omega_{\rm dm}$ | $26.29\%$ | $26.715\%$ | $26.8\%$ | $0.085\%$ |
| Neutrino mass-squared ratio $\Delta m^2_{31}/\Delta m^2_{21}$ | $34$ | $33.856$ | $33.831$ | $0.07\%$ |
| Tsirelson bound $S_{\rm max}$ (CHSH) | $2\sqrt{2}$ | $2\sqrt{2} - 1.76\times10^{-6}$ | $2\sqrt{2}$ (within $10^{-3}$) | Not yet testable |
...
- It produces a **definite prediction** ($\Delta S \approx -1.76 \times 10^{-6}$) rather than a qualitative consistency check.
...
| CHSH optimization at $\pi/8$ | Standard QM | Proved |
...
| Third independent observable (Tsirelson bound) | **Closed** — $\Delta S = -1.76 \times 10^{-6}$ predicted |
...
> The Tsirelson bound $2\sqrt{2}$ is derived as a substrate output of the BCC bipartite lattice. The mirror operator $M = JK$ with $M^2 = -I$ generates Bell-singlet structure on the directional qubit $\{|L\rangle, |R\rangle\}$ embedded in the 4-bit grammar. $M$ acts through the bipartite update with effective angular reach $3/2$ sectors on the 24-sector ring (derived in the mirror-buffer obligation closure from reversibility, bijectivity, and bipartite symmetry). The peak correlation angle is $(3/2)(\pi/12) = \pi/8$, which is the optimal CHSH measurement separation — yielding $S_{\rm max} = 2\sqrt{2}$ as substrate geometry, not quantum-mechanical axiom. The passive mirror correction $b/72$ shifts the effective reach, producing a predicted deviation $\Delta S \approx -1.76 \times 10^{-6}$ below $2\sqrt{2}$ — a precision prediction not yet within experimental reach. This closes mirror-buffer Obligation 3: three independent sublattice-sensitive observables (dark matter fraction, neutrino mass-squared ratio, Tsirelson bound) now receive the same $b/72$ correction from the same substrate mechanism.
