...
Formal justification for the one-way doubling. A CHSH measurement is a projection onto a definite spin axis in the ${∣L⟩,∣R⟩}\{|L\rangle$, $|R\rangle\}
{∣L⟩,∣R⟩}$ subspace. This projection necessarily engages both chiral components $nLhn_{Lh}$
nLh​ and $nRhn_{Rh}$
nRh​ simultaneously for two independent reasons.

First, bijectivity. The QPRCA update rule is bijective on the full 4-bit state $(nL,nR,bbit,q)(n_L, n_R, b_{\rm bit}, q)$
$(nL​,nR​,bbit​,q)$. In the bifurcated address state $(nL=1,nR=1)(n_L=1, n_R=1)$
$(nL​=1,nR​=1)$, the two directional occupancy bits are jointly load-bearing — they are not independently addressable subsystems. A projection that resolved $nLhn_{Lh}$
nLh​ without engaging $nRhn_{Rh}$
nRh​ would constitute a partial projection on a non-independent subsystem, breaking bijectivity on the full state. The grammar does not permit it.

Second, the symplectic generator JJ
J has no block-diagonal form in the ${∣L⟩,∣R⟩}\{|L\rangle, |R\rangle\}$
${∣L⟩,∣R⟩}$ basis. Every rotation parameterized by the CHSH measurement angle $θ\theta$
θ is generated by the full action of JJ
J — which maps $J∣L⟩=∣R⟩J|L\rangle = |R\rangle$
$J∣L⟩=∣R⟩ and J∣R⟩=−∣L⟩J|R\rangle = -|L\rangle$
$J∣R⟩=−∣L⟩$ simultaneously. There is no chiral-selective subgroup. Joint engagement of both components is algebraically forced.

Both together make the doubling formally forced: the substrate bidirectional peak $π/8\pi/8
π/8$ is the per-component reach; a CHSH measurement necessarily resolves both components, doubling the effective angular span to $π/4\pi/4
π/4$.
