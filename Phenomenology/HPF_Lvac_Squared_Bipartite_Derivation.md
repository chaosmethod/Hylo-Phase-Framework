...
**Date:** 2026-05-03 (v4 derivation); 2026-05-05 (consolidation); 2026-05-08 (§ 5.1 L_vac/L_Λ notational clarification)  
...
**Notational clarification (substrate vs macroscopic; added 2026-05-08).** The proportionality "$\Lambda \propto 1/L_{\mathrm{vac}}^{2}$" is a substrate-level scaling-law statement: it establishes that the bipartite multiplicity correction is sourced at the substrate-level area-like observable $L_{\mathrm{vac}}^{2}$, where the canonical 4-bit grammar acts. The dimensional content of $\Lambda$ is carried at the **macroscopic** scale $L_{\Lambda}$ (Volume IV § 12; Symbol Index § 8 entry), with $L_{\Lambda} \approx 9.6 \times 10^{25}\,\mathrm{m}$ versus the substrate $L_{\mathrm{vac}} \approx 1.45 \times 10^{-20}\,\mathrm{m}$ — a separation of ~46 orders of magnitude bridged by the radial law $L_{\mathrm{vac}} = R_H / \varphi^{n}$. The (1 − 4b/72) correction, sourced at $L_{\mathrm{vac}}^{2}$ by the multiplicity argument below (§§ 5.2–5.5), propagates to $\Lambda$ through the inverse-square relationship as $\Lambda_{\mathrm{corrected}} = \Lambda_{\mathrm{raw}} / (1 - 4\,b/72)$ (§ 7.1) without requiring $L_{\mathrm{vac}}$ and $L_{\Lambda}$ to be identified. External cross-cites that set "$L_{\mathrm{vac}} = l_H$" (e.g. Pascher Doc 206 § 8.2) are using "$L_{\mathrm{vac}}$" as a stand-in for $L_{\Lambda}$; that convention recovers the Friedmann form $\Lambda \cdot l_H^{2} = 3\,\Omega_{\Lambda}$ but does not identify HPF's substrate-native $L_{\mathrm{vac}}$.
