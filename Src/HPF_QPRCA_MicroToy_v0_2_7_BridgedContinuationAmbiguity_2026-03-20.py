#!/usr/bin/env python3
"""
HPF_QPRCA_Executable_v0_2_7_BridgedContinuationAmbiguity_2026-03-20.py

Canonical March 20 executable reference aligned to the enriched HPF master
volumes, promoted with the grouped-L2 intermediate aggregation law, a reversible
TQEQ replacement, and a tuned derived-zeta recovery layer.

EPISTEMIC UPGRADE (2026-04-09):
The 4-bit site alphabet (nL, nR, b, q) is no longer classified as a "toy"
harness. It is the candidate-locked fundamental operator grammar of the
substrate. The 16 resulting configurations (15 active non-vacuum microstates)
are mathematically required to close the Fine Structure Constant candidate
baseline through N_cycle = 24 sectors × 15 active microstates = 360.

What this file does:
- Locks the 4-bit alphabet as the foundational state-space grammar.
...
- Replaces the placeholder zeta proxy with a derived-zeta recovery layer on the
  active flux corridor markers 1.05 / 1.3806 / 5.7889.
- Treats route burden as a live current-canon burden input while strongest-form
  microscopic route subfactors remain open.
- Provides built-in demos and CLI output.

Compatibility note:
- The filename is retained for repo stability.
- The historical class name QPRCAMicroToyV020 is retained for compatibility with
  qprca.py and downstream wrappers.
"""
...
class BridgeConfig:
    ...
    # Derived-zeta recovery weights and live flux-axis markers.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Canonical HPF/QPRCA microtoy v0.2.7 bridged continuation-ambiguity")
    ...
    parser.add_argument("--aggregate-mode", default="grouped_l2", choices=["additive", "max", "grouped_l2"])
